...
and a cycle through values, a cell whose value holds a handle upstream
...
## Nodes Live in an Arena, Handles Are Tokens

Every node lives in one arena owned by the `Graph`, indexed by `u32`.
There is no `Rc` and no `RefCell` anywhere in the graph. A handle is a
token: index, generation, graph id. A freed slot bumps its generation,
...
move-only because it is linear (RFD 4); it is still three words.
...
such a value), and the explicit `depends` declarations below.
...
Why not reference counts. Handle counts root what the I/O world holds
and what values hold, but they cannot see cycles; `sodium-rust`'s
...
persist. Stream occurrence types need nothing beyond what their stages
...
declares it: `b.depends(&node, &token)` states that a node keeps a
token alive, callable after any construction, taking references so it
does not consume a linear stream. Upstream captures need no
...
never pays. A manual policy is available, with `graph.collect()`, for
...
`Listener` and `Pin` are RAII: dropping the handle unlistens or unpins
by flipping a flag the node shares, so dropping one inside a listener
...
modes (RFD 5). Sampling a collected cell must return something, so it
always fails.
