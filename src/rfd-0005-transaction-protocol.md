...
Listeners have no graph access (RFD 2), so whether they run before or
...
I/O operations come in pairs following the standard library: `send`
and `try_send`, `listen` and `try_listen`, and so on. The panicking
variant panics on misuse in both build modes, with one class excepted.
An operation on a collected node whose effect is unobservable by the
semantics, meaning sending to a collected input, listening to a
collected stream, or pinning a collected node, is a debug-mode panic
and a release-mode no-op in the panicking variant, following the
integer-overflow precedent, and is counted on the graph so a release
build can report that it is dropping sends. The `try_` variants
return `Err(Stale)` in both modes. Sampling a collected cell must
return something, so it panics or returns `Err`. A double send is a
violation with an observable outcome either way, so it is an error in
both modes; last-wins in release would mean production behaves
differently from every test that ever ran. A `Remote::send` from graph
code is in the same class (RFD 6).
