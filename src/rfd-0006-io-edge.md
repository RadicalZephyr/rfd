...
and mailbox adapters from `listen` into its channel types. Multiple
...
build modes, `Err` from `try_send`, since it is a logic error with an
observable outcome and not an unobservable one (RFD 5). From another
